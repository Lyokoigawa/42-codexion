...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/08/08 17:25:35 by marvin            #+#    #+#             */
/*   Updated: 2026/08/08 17:25:35 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    heapify_down(t_heap *heap, int index, int policy)
{
    ...
    if (left_child_index < heap->size)
        smallest_index = higher_child(heap, left_child_index, right_child_index, policy);
...
void    heap_remove(t_simulation *sim, t_request *request)
{
    t_heap *heap;
    int i;

    heap = sim->scheduler.heap;
    if (!heap || heap->size == 0 || !request)
        return ;
    i = 0;
    while (i < heap->size && heap->request[i] != request)
        i++;
    if (i == heap->size)
        return ;
    heap->request[i] = heap->request[heap->size - 1];
    heap->size--;
    if (i > 0 && is_higher(heap->request[i], heap->request[(i - 1) / 2], sim->input->scheduler))
        heapify_up(heap, i, sim->input->scheduler);
    else
        heapify_down(heap, i, sim->input->scheduler);
}
